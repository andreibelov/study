CREATE TABLE POST (ID BIGINT NOT NULL, CONTENT VARCHAR, CREATED LONGVARBINARY, LIKES BIGINT, PROFILEID BIGINT, UUID LONGVARBINARY NOT NULL UNIQUE, PRIMARY KEY (ID));
CREATE TABLE IMESSAGE (ID BIGINT NOT NULL, CHATROOM LONGVARBINARY, CONTENT VARCHAR, CREATED LONGVARBINARY, ISREAD BOOLEAN, UUID LONGVARBINARY NOT NULL UNIQUE, PRIMARY KEY (ID));
CREATE TABLE USER (ID BIGINT IDENTITY NOT NULL, DTYPE VARCHAR(31), ACCESSLEVEL INTEGER, EMAIL VARCHAR NOT NULL UNIQUE, LOGIN VARCHAR UNIQUE, PASSWORD VARCHAR, PRIMARY KEY (ID));
...
CREATE TABLE RELATION (ID VARCHAR NOT NULL, IDREQUESTEE BIGINT, IDREQUESTER BIGINT, STATUS INTEGER, PRIMARY KEY (ID));
CREATE TABLE AUTHTOKEN (ID BIGINT NOT NULL, KEY VARCHAR NOT NULL UNIQUE, VALUE BIGINT, PRIMARY KEY (ID));
...
CREATE TABLE CHATROOM (ID BIGINT NOT NULL, DESCRIPTION VARCHAR, UUID LONGVARBINARY NOT NULL UNIQUE, PRIMARY KEY (ID));
CREATE TABLE Post_ATTACHMENTS (TYPE VARCHAR, URI LONGVARBINARY, UUID LONGVARBINARY NOT NULL UNIQUE, Post_ID BIGINT);
CREATE TABLE IMessage_ATTACHMENTS (TYPE VARCHAR, URI LONGVARBINARY, UUID LONGVARBINARY NOT NULL UNIQUE, IMessage_ID BIGINT);
